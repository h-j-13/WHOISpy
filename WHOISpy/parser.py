...
todo : finish the doc and test it
"""

import re


# def com_manage(data, __domain_whois):
#     """存在二级服务器的 提取函数"""
#     if __domain_whois['sec_whois_server'] != '':
#         sec_func_name = Resource.WhoisFunc.get_whois_func(__domain_whois['sec_whois_server'])
#         if sec_func_name is not None:  # 存在专用的二级服务器处理函数
#             if sec_func_name == 'com_manage':  # 防止递归
#                 sec_func_name = 'general_manage'
#             __domain_whois = eval('{func}(data, __domain_whois)'.format(func=sec_func_name))
#     else:
#         __domain_whois = general_manage(data, __domain_whois)
#     return __domain_whois
